...
const signin = (authprovider: AuthenticationProvider, context: ExtensionContext): Command => {
  return (
    {
      command: "tabby.signin",
      callback: async () => {
...
const newChatSession = (chatprovider: ChatViewProvider): Command => {
  return (
    {
      command: "tabby.newchat",
      callback: async () => {
        chatprovider.startNewChatSession()
      }
    }
  );
}

const previousSessions = (chatprovider: ChatViewProvider): Command => {
  return (
    {
      command: "tabby.previous",
      callback: async () => {
        chatprovider.retrieveAndDisplayPreviousSessions()
      }
    }
  );
}
...
export const tabbyCommands = (
  ...
  chatViewProvicer: ChatViewProvider
) =>
  [
    ...
    newChatSession(chatViewProvicer),
    previousSessions(chatViewProvicer),
